...
class UserController extends Controller
{
    ...
    {
    	$this->_data['items'] = User::where('id','!=',1)->orderBy('priority','asc')->orderBy('id','desc')->paginate(25);
    ...
    public function store(Request $request){
        ...
        if ($valid->fails()) {
            ...
            return redirect()->route('admin.user.index')->with('success', 'Thêm người dùng <b>'. $user->name .'</b> thành công');
        }
    }
    ...
    public function delete($id){
        ...
        if ($user !== null) {
            $user->delete();
            return redirect()->route('admin.user.index')->with('success', 'Xóa người dùng <b>'. $user->name .'</b> thành công');
        }
        return redirect()->route('admin.user.index')->with('danger', 'Không tìm thấy người dùng này');
    }

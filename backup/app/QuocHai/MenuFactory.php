...
class MenuFactory{
	...
	public function getMenu($parent=0,$lvl=0){
		if( isset($this->_data[$parent]) ){
			...
			foreach($this->_data[$parent] as $k=>$v){
				...
				$id=$v->id;
				$slug=$v->slug;
				$this->_result.= "<a href='$this->_baseurl/$slug' data-lvl='$lvl'>".$icon.$v->title."</a>";
				$this->getMenu($id,++$lvl);
	...
	public function getMenuSelectLimit($self=0,$parent=0,$prefix='',$select=0,$level=1){
		...
		if( isset($this->_data[$parent]) ){
			foreach($this->_data[$parent] as $key => $val){
				...
				if( $id != $self ){
					if( $id == $select ) $selected = "selected"; else $selected = "";
					if( $this->_siteconfig[$_GET['type']]['level'] > $level ) $disabled = ""; else $disabled = "disabled";
				...
				if( ($id != $self) && ($val->parent == $parent) ){
					...
					$this->_result .= '
						<tr id="record-'.$id.'" '.$isParent.'>
	                        ...
	                        <td align="left"> <a href="'.route('category.edit',[$id, 'type'=>$_GET['type']]).'"> '.$prefix.' '.$name.' </a> </td>
	                        '.$hasImage.'
	                        <td align="center"> '.$val->created_at.' </td>
	                        <td align="center"> '.$button.' </td>
	                        <td align="center">
	                            <a href="'.route('category.edit',[$id, 'type'=>$_GET['type']]).'" class="btn btn-sm blue" title="Chỉnh sửa"> <i class="fa fa-edit"></i> </a>
	                            <form action="'.route('category.delete',[$id, 'type'=>$_GET['type']]).'" method="post">
	                                '.csrf_field().'
	                                '.method_field('DELETE').'

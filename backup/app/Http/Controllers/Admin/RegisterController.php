...
class RegisterController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if ($valid->fails()) {
            ...
            $register->save();
            return redirect()->route('register.index',['type'=>$this->_data['type']])->with('success','Thêm dữ liệu <b>'.$register->name.'</b> thành công');
        }
    ...
    public function edit($id){
        ...
        return redirect()->route('register.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }
    ...
    public function delete($id){
        ...
        if ($register !== null) {
            if( $register->delete() ){
                return redirect()->route('register.index',['type'=>$this->_data['type']])->with('success', 'Xóa dữ liệu <b>'.$deleted.'</b> thành công');
            }else{
                return redirect()->route('register.index',['type'=>$this->_data['type']])->with('danger', 'Xóa dữ liệu bị lỗi');
            }
        }
        return redirect()->route('register.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }

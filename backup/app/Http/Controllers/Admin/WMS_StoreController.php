...
class WMS_StoreController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if ($valid->fails()) {
            ...
            $wms_store->save();
            return redirect()->route('wms_store.index',['type'=>$this->_data['type']])->with('success','Thêm dữ liệu <b>'.$wms_store->name.'</b> thành công');
        }
    ...
    public function edit($id){
        ...
        return redirect()->route('wms_store.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }
    ...
    public function delete($id){
        ...
        if ($wms_store !== null) {
            if( $wms_store->delete() ){
                return redirect()->route('wms_store.index',['type'=>$this->_data['type']])->with('success', 'Xóa dữ liệu <b>'.$deleted.'</b> thành công');
            }else{
                return redirect()->route('wms_store.index',['type'=>$this->_data['type']])->with('danger', 'Xóa dữ liệu bị lỗi');
            }
        }
        return redirect()->route('wms_store.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }

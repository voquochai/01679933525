...
class CommentController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if ($valid->fails()) {
            ...
            $comment->save();
            return redirect()->route('comment.index',['type'=>$this->_data['type']])->with('success','Thêm dữ liệu <b>'.$comment->name.'</b> thành công');
        }
    ...
    public function edit($id){
        ...
        return redirect()->route('comment.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }
    ...
    public function delete($id){
        ...
        if ($comment !== null) {
            if( $comment->delete() ){
                return redirect()->route('comment.index',['type'=>$this->_data['type']])->with('success', 'Xóa dữ liệu <b>'.$deleted.'</b> thành công');
            }else{
                return redirect()->route('comment.index',['type'=>$this->_data['type']])->with('danger', 'Xóa dữ liệu bị lỗi');
            }
        }
        return redirect()->route('comment.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }

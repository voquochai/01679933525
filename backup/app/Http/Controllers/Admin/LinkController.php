...
class LinkController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if ($valid->fails()) {
            ...
            return redirect()->route('link.index',['type'=>$this->_data['type']])->with('success','Thêm dữ liệu <b>'.$link->languages[0]->title.'</b> thành công');
        }
    ...
    public function edit($id){
        ...
        return redirect()->route('link.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }
    ...
    public function delete($id){
        ...
        if ($link !== null) {
            delete_image($this->_data['path'].'/'.$link->image,$this->_data['thumbs']);
            if( $link->delete() ){
                return redirect()->route('link.index',['type'=>$this->_data['type']])->with('success', 'Xóa dữ liệu <b>'.$deleted.'</b> thành công');
            }else{
                return redirect()->route('link.index',['type'=>$this->_data['type']])->with('danger', 'Xóa dữ liệu bị lỗi');
            }
        }
        return redirect()->route('link.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }

...
class PhotoController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if ($valid->fails()) {
            ...
            return redirect()->route('photo.index',['type'=>$this->_data['type']])->with('success','Thêm dữ liệu <b>'.$photo->languages[0]->title.'</b> thành công');
        }
    ...
    public function edit($id){
        ...
        return redirect()->route('photo.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }
    ...
    public function delete($id){
        ...
        if ($photo !== null) {
            delete_image($this->_data['path'].'/'.$photo->image,$this->_data['thumbs']);
            if( $photo->delete() ){
                return redirect()->route('photo.index',['type'=>$this->_data['type']])->with('success', 'Xóa dữ liệu <b>'.$deleted.'</b> thành công');
            }else{
                return redirect()->route('photo.index',['type'=>$this->_data['type']])->with('danger', 'Xóa dữ liệu bị lỗi');
            }
        }
        return redirect()->route('photo.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }

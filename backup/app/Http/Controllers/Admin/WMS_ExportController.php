...
class WMS_ExportController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if ($valid->fails()) {
            ...
            $wms_export->save();
            return redirect()->route('wms_export.index',['type'=>$this->_data['type']])->with('success','Thêm dữ liệu <b>'.$wms_export->code.'</b> thành công');
        }
    ...
    public function edit($id){
        ...
        return redirect()->route('wms_export.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }
    ...
    public function delete($id){
        ...
        if ($wms_export !== null) {
            if( $wms_export->delete() ){
                return redirect()->route('wms_export.index',['type'=>$this->_data['type']])->with('success', 'Xóa dữ liệu <b>'.$deleted.'</b> thành công');
            }else{
                return redirect()->route('wms_export.index',['type'=>$this->_data['type']])->with('danger', 'Xóa dữ liệu bị lỗi');
            }
        }
        return redirect()->route('wms_export.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }

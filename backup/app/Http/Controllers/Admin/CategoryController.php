...
class CategoryController extends Controller
{
    ...
    public function index()
    {
        ...
        // $this->_data['items'] = Category::where('type',$this->_data['type'])->orderBy('priority','asc')->orderBy('id','desc')
        //        ->with(['languages' => function($query){
        //            $query->select('category_id','title')->where( 'language', $this->_data['default_language'] );
        //        }])->paginate(25);
    ...
    public function store(Request $request){
        ...
        if ($valid->fails()) {
            ...
            return redirect()->route('category.index',['type'=>$this->_data['type']])->with('success','Thêm dữ liệu <b>'.$category->languages[0]->title.'</b> thành công');
        }
    }
    ...
    public function edit($id){
        ...
        return redirect()->route('category.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }
    ...
    public function delete($id){
        ...
        if ($category !== null) {
            ...
            if( $category->delete() ){
                ...
                return redirect()->route('category.index',['type'=>$this->_data['type']])->with('success', 'Xóa dữ liệu <b>'.$deleted.'</b> thành công');
            }else{
                return redirect()->route('category.index',['type'=>$this->_data['type']])->with('danger', 'Xóa dữ liệu bị lỗi');
            }
        }
        return redirect()->route('category.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }

...
class WMS_ImportController extends Controller
{
    ...
    public function ajax(Request $request){
        if($request->ajax()){
            ...
            $items = DB::table('wms_imports')
                ...
                ->whereRaw('FIND_IN_SET(\''.$request->q.'\',product_code)')
            ...
            if( $items !== null ){
                foreach( $items as $item ){
                    ...
                    foreach($product_id as $key => $id){
                        ...
                        $products[$key]['title']    =  @$product->title.' - '.@$color->title.' - '.@$size->title;
    ...
    public function store(Request $request){
        ...
        if ($valid->fails()) {
            ...
            $wms_import->save();
            return redirect()->route('wms_import.index',['type'=>$this->_data['type']])->with('success','Thêm dữ liệu <b>'.$wms_import->code.'</b> thành công');
        }
    ...
    public function edit($id){
        ...
        return redirect()->route('wms_import.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }
    ...
    public function delete($id){
        $wms_import = WMS_Import::find($id);
        $deleted = $wms_import->code;
        if ($wms_import !== null) {
            if( $wms_import->delete() ){
                return redirect()->route('wms_import.index',['type'=>$this->_data['type']])->with('success', 'Xóa dữ liệu <b>'.$deleted.'</b> thành công');
            }else{
                return redirect()->route('wms_import.index',['type'=>$this->_data['type']])->with('danger', 'Xóa dữ liệu bị lỗi');
            }
        }
        return redirect()->route('wms_import.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }

...
class SupplierController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if ($valid->fails()) {
            ...
            return redirect()->route('supplier.index',['type'=>$this->_data['type']])->with('success','Thêm dữ liệu <b>'.$supplier->name.'</b> thành công');
        }
    ...
    public function edit($id){
        ...
        return redirect()->route('supplier.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }
    ...
    public function delete($id){
        ...
        if ($supplier !== null) {
            if( $supplier->delete() ){
                return redirect()->route('supplier.index',['type'=>$this->_data['type']])->with('success', 'Xóa dữ liệu <b>'.$deleted.'</b> thành công');
            }else{
                return redirect()->route('supplier.index',['type'=>$this->_data['type']])->with('danger', 'Xóa dữ liệu bị lỗi');
            }
        }
        return redirect()->route('supplier.index',['type'=>$this->_data['type']])->with('danger', 'Dữ liệu không tồn tại');
    }

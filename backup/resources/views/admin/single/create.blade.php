...
@section('breadcrumb')
<li>
    <a href="{{ route('single.index', ['type'=>$type]) }}"> {{ $pageTitle }} </a>
...
<div class="row">
    ...
    <!-- BEGIN FORM-->
    <form role="form" method="POST" action="{{ route('single.store',['type'=>$type]) }}" enctype="multipart/form-data" >
        {{ csrf_field() }}
                    ...
                    <div class="tab-content">
                        ...
                        <div class="tab-pane {!! (( $key==$default_language )?'active':'') !!}" id="tab_{{ $key }}">
                            ...
                            <div class="form-group">
                                ...
                                <div class="ck-editor">
                                    <textarea name="dataL[{{ $key }}][contents]" id="contents_{{ $key }}" class="form-control" rows="6">{{ old('dataL.'.$key.'.contents') }}</textarea>
                                </div>
                            </div>
                            @endif
                            ...
                            <div class="form-group">
                                ...
                                <div>
                                    <input type="text" name="dataL[{{ $key }}][meta_seo][title]" class="form-control" value="{{ old('dataL.'.$key.'.meta_seo.title') }}">
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div>
                                    <textarea name="dataL[{{ $key }}][meta_seo][keywords]" class="form-control" rows="6">{{ old('dataL.'.$key.'.meta_seo.keywords') }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div>
                                    <textarea name="dataL[{{ $key }}][meta_seo][description]" class="form-control" rows="6">{{ old('dataL.'.$key.'.meta_seo.description') }}</textarea>
                                </div>
                            </div>

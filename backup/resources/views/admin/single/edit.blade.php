...
<div class="row">
    ...
    <!-- BEGIN FORM-->
    <form role="form" method="POST" action="{{ route('single.update',['id'=>$item->id,'type'=>$type]) }}" enctype="multipart/form-data" >
        {{ csrf_field() }}
        {{ method_field('put') }}
                        ...
                        <div>
                            <div class="fileinput {{ ( $item->image && public_path( get_thumbnail($folder.'/'.$item->image) ) ) ? 'fileinput-exists' : 'fileinput-new' }}" data-provides="fileinput">
                                ...
                                <div>
                                    ...
                                    <a href="javascript:;" class="btn btn-delete-file default fileinput-exists" data-dismiss="fileinput" data-ajax="act=delete_file|table=singles|id={{ $item->id }}|config=single|type={{ $type }}"> Xóa </a>
                                </div>
                            </div>
                        </div>
                    </div>

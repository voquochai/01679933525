...
@section('breadcrumb')
<li>
    <a href="{{ route('post.index', ['type'=>$type]) }}"> {{ $pageTitle }} </a>
...
<div class="row">
    ...
    <!-- BEGIN FORM-->
    <form role="form" method="POST" action="{{ route('post.update',['id'=>$item->id,'type'=>$type]) }}" enctype="multipart/form-data" >
        {{ csrf_field() }}
        {{ method_field('put') }}
                    ...
                    <div class="form-group">
                        ...
                        <div>
                            <select name="tags[]" class="selectpicker show-tick show-menu-arrow form-control" multiple>
                                @forelse($tags as $tag)
                                <option value="{{ $tag->id }}" {{ ( in_array($tag->id,$item->getIdsAttribute()) ) ? 'selected' : '' }} > {{ $tag->title }} </option>
                                @empty
                                @endforelse
                            </select>
                        </div>
...
<div id="tags-modal" class="modal fade" tabindex="-1" data-focus-on="input:first">
    <form role="form" method="POST" action="#">
        ...
        <div class="modal-footer">
            <button type="button" data-dismiss="modal" class="btn default">Thoát</button>
            <button type="button" class="btn green btn-quick-add" data-ajax="tags[]" data-url="{{ route('attribute.store',['type'=>'post_tags']) }}"> <i class="fa fa-check"></i> Lưu</button>
        </div>
    </form>
</div>
@endif

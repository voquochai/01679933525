	...
	<div class="col-md-12">
		<div class="portlet box green">
			<div class="portlet-title">
                ...
                <div class="actions">
                    <a href="{{ route('product.create',['type'=>$type]) }}" class="btn btn-sm btn-default"> Thêm mới </a>
                    <div class="btn-group">
					...
					<table class="table table-bordered table-condensed">
						...
						<tbody>
                            ...
                            <tr id="record-{{ $item->id }}">
                                ...
                                @endif
                                <td align="center"><a href="{{ route('product.edit',['id'=>$item->id, 'type'=>$type]) }}"> {{ $item->title }} </a></td>
                                @if($siteconfig[$type]['image'])
                                ...
                                <td align="center">
                                    <a href="{{ route('product.edit',['id'=>$item->id, 'type'=>$type]) }}" class="btn btn-sm blue" title="Chỉnh sửa"> <i class="fa fa-edit"></i> </a>
                                    <form action="{{ route('product.delete',['id'=>$item->id, 'type'=>$type]) }}" method="post">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}

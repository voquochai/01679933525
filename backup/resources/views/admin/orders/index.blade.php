	...
	<div class="col-md-12">
		<div class="portlet box green">
			<div class="portlet-title">
                ...
                <div class="actions">
                    <a href="{{ route('order.create',['type'=>$type]) }}" class="btn btn-sm btn-default"> Thêm mới </a>
                    <div class="btn-group">
					...
					<table class="table table-bordered table-condensed">
						...
						<tbody>
                            ...
                            <tr id="record-{{ $item->id }}">
                                ...
                                <td align="center"><a href="{{ route('order.edit',['id'=>$item->id, 'type'=>$type]) }}"> {{ $item->name.' - '.$item->phone }} </a></td>
                                ...
                                <td align="center">
                                    <a href="{{ route('order.edit',['id'=>$item->id, 'type'=>$type]) }}" class="btn btn-sm blue" title="Chỉnh sửa"> <i class="fa fa-edit"></i> </a>
                                    <form action="{{ route('order.delete',['id'=>$item->id, 'type'=>$type]) }}" method="post">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
